			...
			if sheet.cell(i,5).value == '[低]':  #去掉漏洞等级为低的漏洞
				...
				if (sheet.cell(i,0).value == ''):
					...
					try:
						...
						是否部委备案系统  = '\''+'未查询到相关信息'+'\''
						定级备案登记  = '\''+'未查询到相关信息'+'\''
					cursor.execute("UPDATE %s SET 系统名称 = %s WHERE IP地址 = %s" % (db_table_name,系统名称,IP地址));
					cursor.execute("UPDATE %s SET 维护科室  = %s WHERE IP地址 = %s" % (db_table_name,维护科室 ,IP地址));
					cursor.execute("UPDATE %s SET 中心责任人姓名 = %s WHERE IP地址 = %s" % (db_table_name,中心责任人姓名,IP地址));
					cursor.execute("UPDATE %s SET 中心责任人邮箱 = %s WHERE IP地址 = %s" % (db_table_name,中心责任人邮箱,IP地址));
					cursor.execute("UPDATE %s SET 中心责任人电话 = %s WHERE IP地址 = %s" % (db_table_name,中心责任人电话,IP地址));
					cursor.execute("UPDATE %s SET 厂商联系人姓名 = %s WHERE IP地址 = %s" % (db_table_name,厂商联系人姓名,IP地址));
					cursor.execute("UPDATE %s SET 厂商联系人电话 = %s WHERE IP地址 = %s" % (db_table_name,厂商联系人电话,IP地址));
					cursor.execute("UPDATE %s SET 是否部委备案系统 = %s WHERE IP地址 = %s" % (db_table_name,是否部委备案系统,IP地址));
					cursor.execute("UPDATE %s SET 定级备案登记 = %s WHERE IP地址 = %s" % (db_table_name,定级备案登记,IP地址));
					i = i + 1
				else:
					IP地址= xls_name
					print(IP地址)
...
def main(DB,xls_dir,yaoxierudebiao,zichan):
	...
	for file in files:
		...
		zichan = zichan
		loudong_xieru(xls_dir + '\\' + file[0:-4], yaoxierudebiao,zichan)  # 这里设置要写入的表名，月份
		p = p + 1  # 测试导入表格的数量
...
if __name__ == "__main__":
	main(r'F:/Tong.db',r'F:\扫描报告','\'' + '漏洞_1901' + '\'','\'' + '资产_1901' + '\'')
	#需要已写入四个参数：漏洞数据库db文件  要写入的xls文件  漏洞库表  资产表
...
# # 连接数据库
# # 留档数据库
# # conn = sqlite3.connect('E:\\文件库\\工作\\辽宁移动互联网运营中心\\5数据库\\zichan.db')
# # 测试数据库
# conn = sqlite3.connect('E:\\python\\新版绿盟漏洞统计脚本-sqlite\\liantongloud.db')
# cursor = conn.cursor()
#
#
# # 获取指定文件夹下的文件名称++++++++++++++++++++++++++++++++++++++<
...
# 					返回信息 = sheet.cell(i, 13).value
# 					cursor.execute(
# 						'''INSERT INTO ''' + db_table_name + ''' (IP地址,端口,协议,服务,漏洞名称,漏洞风险值,风险等级,发现日期,CVE编号,CNNVD编号,CNCVE编号,CNVD编号,详细描述,解决办法,返回信息) values(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)''',
# 						(IP地址, 端口, 协议, 服务, 漏洞名称, 漏洞风险值, 风险等级, 发现日期, CVE编号, CNNVD编号, CNCVE编号, CNVD编号, 详细描述, 解决办法, 返回信息))
# 					# cursor.execute('''INSERT INTO '''+db_table_name+''' (IP地址,端口,协议,服务,漏洞名称,漏洞风险值,风险等级,发现日期,CVE编号,CNNVD编号,CNCVE编号,CNVD编号,详细描述,解决办法,返回信息) values(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)''',(IP地址,端口,协议,服务,漏洞名称,漏洞风险值,风险等级,发现日期,CVE编号,CNNVD编号,CNCVE编号,CNVD编号,详细描述,解决办法,返回信息))
#
# 					# 导入科室，业务系统，公网or内网和负责人
# 					zichan = zichan
...
#
# # 获取所有扫描结果文件名
# file_dir = 'E:\python\新版绿盟漏洞统计脚本-sqlite\ceshi'  # 将要导入的文件发在这个文件夹下
#
# files = file_name(file_dir)
# print(files)
#
...
# for file in files:
# 	print(file[0:-4])
# 	# yaoxierudebiao = guishu(file)
# 	# print(yaoxierudebiao + file)
# 	yaoxierudebiao = '\'' + '漏洞库-联通7月' + '\''
# 	zichan = '\'' + '资产表-联通7月' + '\''
