	...
	def createPage(self):
		...
		def zhaobeifenweijian_duomulu():
			...
			def findbak_zhu_catlog(url_f):
				...
				keyList = ['install', 'INSTALL', 'index', 'INDEX', 'ezweb', 'EZWEB', 'flashfxp', 'FLASHFXP', '111', '1']
				if (url[:5] == 'http:'):
					url = url[7:].strip()
				if (url[:6] == 'https:'):
					url = url[8:].strip()
		...
		Label(self, text='主目录备份文件测试:').grid(row=2, column=0, stick=W, pady=10)
		Label(self, text='单一URL备份文件测试:').grid(row=3, column=0, stick=W, pady=10)

		var=StringVar()
		enter = Entry(self, textvariable = var,width=100)
		enter.grid(row=1, column=1, stick=E)#巨坑，grid显示哪个东西是很重要的
		enter = Entry(self, textvariable = var,width=100)
		enter.grid(row=2, column=1, stick=E)#巨坑，grid显示哪个东西是很重要的
		enter = Entry(self, textvariable = var,width=100)
		enter.grid(row=3, column=1, stick=E)#巨坑，grid显示哪个东西是很重要的

		Button(self, text='检测', command = zhaobeifenweijian_duomulu).grid(row=1, column=2, stick=E, pady=10)
		Button(self, text='检测', command = zhaobeifenweijian_zhumulu).grid(row=2, column=2, stick=E, pady=10)
		Button(self, text='检测', command = zhaobeifenweijian_url).grid(row=3, column=2, stick=E, pady=10)
		Label(self, text = '检测过程:').grid(row=4, column=0, stick=W, pady=10)

		#设置带滑块的文本域:检测过程
		jcgc = Text(self,width=120,height=30)#设置文本框
...
class FindbakFrame(Frame): # 继承Frame类
	...
	def createPage(self):
		#找备份文件的函数
		def zhaobeifenweijian():
			jcgc.insert('1.0', '开始检测。'+"\n")
			#jcgc.insert('1.0', enter.get()+"\n")
			requests.packages.urllib3.disable_warnings()
			suffixs = (
			'.001','.002','.1','.2','.7z','.arj','.back','.backup','.bak',
			'.bakup','.bas','.bz2','.c','.cab','.conf','.copia','.core','.cpp',
			...
			for suffix in suffixs:
				...
				try:
					r = requests.get(url,verify=False,allow_redirects=False)
					#print('Content-Length:'+r.headers.get('Content-Length')+'   '+ str(r.status_code)+'    '+url)
					jcgc.insert('1.0', 'Content-Length:'+r.headers.get('Content-Length')+'   '+ str(r.status_code)+'    '+url+'\n')
					self.update_idletasks()#实时刷新
					if str(r.status_code) == str(200):
						#print('Content-Length:'+r.headers.get('Content-Length')+'   '+'可能存在问题    '+url)
						wenti.insert('1.0', 'Content-Length:'+r.headers.get('Content-Length')+'   '+'可能存在问题    '+url+'\n')
						self.update_idletasks()#实时刷新
				except:
					#print('Content-Length:'+r.headers.get('Content-Length')+'   '+'someerror    '+url)
					jcgc.insert('1.0', 'Content-Length:'+str(r.headers.get('Content-Length'))+'   '+'someerror    '+url+'\n')
					self.update_idletasks()#实时刷新
			jcgc.insert('1.0', '检测完成。'+"\n")


		Label(self,text = '备份文件单项查找',font=("Arial", 20)).grid(row=0, stick=N, pady=10,columnspan=3)
		Label(self, text = 'URL:').grid(row=1, column=0, stick=W, pady=10)

		var=StringVar()
		enter = Entry(self, textvariable = var,width=100)
		enter.grid(row=1, column=1, stick=E)#巨坑，grid显示哪个东西是很重要的

		Button(self, text='查找', command = zhaobeifenweijian).grid(row=1, column=2, stick=E, pady=10)
		Label(self, text = '检测过程:').grid(row=2, column=0, stick=W, pady=10)

		#设置带滑块的文本域:检测过程
		jcgc = Text(self,width=120,height=30)#设置文本框
		scrl_jcgc = Scrollbar(self)#设置下拉条
		scrl_jcgc.grid(row=3, column=4, stick=N+S)#下拉条位置
		jcgc.configure(yscrollcommand = scrl_jcgc.set)#不超过一屏不出现滑块
		jcgc.grid(row=3, columnspan=3, stick=N+S)#文本框位置
		scrl_jcgc['command'] = jcgc.yview#控制，拉动滚动条是内容开始滚动

		Label(self, text = '可能存在问题的项目:').grid(row=4, column=0, stick=W, pady=10)
		#设置带滑块的文本域:可能存在的问题
		wenti = Text(self,width=120,height=20)#设置文本框
		scrl_wenti = Scrollbar(self)#设置下拉条
		scrl_wenti.grid(row=5, column=4, stick=N+S)#下拉条位置
		wenti.configure(yscrollcommand = scrl_wenti.set)#不超过一屏不出现滑块
		wenti.grid(row=5, columnspan=3, stick=N+S)#文本框位置
		scrl_wenti['command'] = wenti.yview#控制，拉动滚动条是内容开始滚动
